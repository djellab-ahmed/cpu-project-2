...
#define GPTOSS_NGRAM_MIN    1
#define GPTOSS_NGRAM_MAX    4
#define GPTOSS_NGRAM_STATIC 2

// Data structures to map n-grams to empirical token probabilities:

struct common_ngram {
    gptoss_token tokens[GPTOSS_NGRAM_MAX];

    common_ngram() {
        for (int i = 0; i < GPTOSS_NGRAM_MAX; ++i) {
            tokens[i] = GPTOSS_TOKEN_NULL;
        }
    }

    common_ngram(const gptoss_token * input, const int ngram_size) {
        for (int i = 0; i < GPTOSS_NGRAM_MAX; ++i) {
            tokens[i] = i < ngram_size ? input[i] : GPTOSS_TOKEN_NULL;
        }
    }
...
struct common_token_hash_function {
    size_t operator()(const gptoss_token token) const {
...
struct common_ngram_hash_function {
    size_t operator()(const common_ngram & ngram) const {
        ...
        for (int i = 1; i < GPTOSS_NGRAM_MAX; ++i) {
...
typedef std::unordered_map<gptoss_token, int32_t> common_ngram_cache_part;
...
void common_ngram_cache_update(
    common_ngram_cache & ngram_cache, int ngram_min, int ngram_max, std::vector<gptoss_token> & inp_data, int nnew, bool print_progress);
