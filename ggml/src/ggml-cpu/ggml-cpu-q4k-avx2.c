...
static GGML_TLS float * tls_decode_wtile = NULL;
static GGML_TLS size_t  tls_decode_wtile_cap = 0;
#endif
...
static inline void ggml_qgemv_debug_print(const struct ggml_compute_params * params, const struct ggml_tensor * w, const char * name) {
    const char * debug_env = getenv("GPTOSS_QGEMV_DEBUG");
    if (debug_env != NULL && debug_env[0] != '\0' && params->ith == 0) {
        fprintf(stderr, "[qgemv] type=%d n=1 kernel=%s\n", (int) w->type, name);
    }
}
...
void ggml_mul_mat_q4k_decode_avx2(
    ...
    ggml_qgemv_debug_print(params, w, "q4k-avx2");
...
static inline void ggml_qgemv_debug_print_mxfp4(const struct ggml_compute_params * params, const struct ggml_tensor * w) {
    ggml_qgemv_debug_print(params, w, "mxfp4-avx2");
}
...
void ggml_mul_mat_mxfp4_decode_avx2(
    ...
    ggml_qgemv_debug_print_mxfp4(params, w);
    ...
    float * w_tile = ggml_qgemv_tls_realloc((void **) &tls_decode_wtile, &tls_decode_wtile_cap, (size_t) tile, sizeof(float));
    ...
    for (int64_t row_index = r0; row_index < r1; ++row_index) {
        ...
        for (int64_t off = 0; off < cols; off += tile) {
            ...
            for (; i + 8 <= blk; i += 8) {
                ...
                const __m256 prod = _mm256_mul_ps(vw, vx);
                vacc = _mm256_add_ps(vacc, prod);
            }
            for (; i < blk; ++i) {
                acc_tail += w_tile[i] * x_f32_cur[off + i];
            }
        }

        float acc_vec[8];
        _mm256_storeu_ps(acc_vec, vacc);
        float acc_scalar = acc_tail;
        for (int i = 0; i < 8; ++i) {
            acc_scalar += acc_vec[i];
        }
...
void ggml_mul_mat_mxfp4_decode_avx2(
    ...
    (void) w;
    (void) x;
    GGML_ABORT("MXFP4 decode AVX2 kernel requires AVX2 support");
}
#endif
