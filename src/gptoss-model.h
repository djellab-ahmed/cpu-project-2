...
enum llm_type {
    ...
    LLM_TYPE_57B_A14B,
    LLM_TYPE_17B_16E, // gptoss4 Scout
    LLM_TYPE_17B_128E, // gptoss4 Maverick
    LLM_TYPE_A13B,
    LLM_TYPE_7B_A1B,
...
struct gptoss_layer {
    ...
    struct gptoss_layer_posnet posnet;

    struct gptoss_layer_convnext convnext;

    struct gptoss_layer_shortconv shortconv;

    struct gptoss_layer_nextn nextn;
};
...
struct gptoss_model {
    ...
    std::vector<gptoss_layer> layers;
    ...
    explicit gptoss_model(const struct gptoss_model_params & params);
    ~gptoss_model();

    void load_stats  (gptoss_model_loader & ml);
    void load_arch   (gptoss_model_loader & ml);
    void load_hparams(gptoss_model_loader & ml);
    void load_vocab  (gptoss_model_loader & ml);
    bool load_tensors(gptoss_model_loader & ml); // returns false if cancelled by progress_callback
    ...
    float get_rope_freq_base (const gptoss_cparams & cparams, int il) const;
    float get_rope_freq_scale(const gptoss_cparams & cparams, int il) const;

    ggml_tensor * get_rope_factors(const gptoss_cparams & cparams, int il) const;

    // note: can mutate `cparams`
    // TODO: move this to new llm_arch_model_i interface
    gptoss_memory_i * create_memory(const gptoss_memory_params & params, gptoss_cparams & cparams) const;
