...
static const char * gptoss_expert_gating_func_name(gptoss_expert_gating_func_type type) {
    switch (type) {
        case GPTOSS_EXPERT_GATING_FUNC_TYPE_SOFTMAX: return "softmax";
        case GPTOSS_EXPERT_GATING_FUNC_TYPE_SIGMOID: return "sigmoid";
        default:                                    return "unknown";
    }
}

static const std::map<gptoss_rope_scaling_type, const char *> GPTOSS_ROPE_SCALING_TYPES = {
    { GPTOSS_ROPE_SCALING_TYPE_NONE,       "none"       },
    { GPTOSS_ROPE_SCALING_TYPE_LINEAR,     "linear"     },
    { GPTOSS_ROPE_SCALING_TYPE_YARN,       "yarn"       },
    { GPTOSS_ROPE_SCALING_TYPE_LONGROPE,   "longrope"   },
};

std::string gptoss_rope_scaling_type_name(gptoss_rope_scaling_type rope_scaling_type) {
    return GPTOSS_ROPE_SCALING_TYPES.at(rope_scaling_type);
}

static gptoss_rope_scaling_type gptoss_rope_scaling_type_from_string(const std::string & name) {
    for (const auto & kv : GPTOSS_ROPE_SCALING_TYPES) {
        if (kv.second == name) {
            return (gptoss_rope_scaling_type) kv.first;
        }
    }

    return GPTOSS_ROPE_SCALING_TYPE_UNSPECIFIED;
}

// checks if the weight tensor can be used with the specified buffer type and device
static bool weight_buft_supported(const gptoss_hparams & hparams, ggml_tensor * w, ggml_op op, ggml_backend_buffer_type_t buft, ggml_backend_dev_t dev) {
...
static ggml_backend_buffer_type_t select_weight_buft(const gptoss_hparams & hparams, ggml_tensor * tensor, ggml_op op, const buft_list_t & buft_list) {
...
// GPU: split if GPTOSS_SPLIT_MODE_ROW -> GPU
static buft_list_t make_gpu_buft_list(ggml_backend_dev_t dev, gptoss_split_mode split_mode, const float * tensor_split) {
    buft_list_t buft_list;

    // add the device split buffer type if requested and available
    if (split_mode == GPTOSS_SPLIT_MODE_ROW) {
...
struct gptoss_model::impl {
    ...
    gptoss_mmaps mappings;

    // objects representing data potentially being locked in memory
    gptoss_mlocks mlock_bufs;
    gptoss_mlocks mlock_mmaps;
...
gptoss_model::gptoss_model(const gptoss_model_params & params) : params(params), pimpl(std::make_unique<impl>()) {
    pimpl->has_tensor_overrides = params.tensor_buft_overrides && params.tensor_buft_overrides[0].pattern;
}

gptoss_model::~gptoss_model() {}

void gptoss_model::load_stats(gptoss_model_loader & ml) {
    pimpl->n_elements = ml.n_elements;
    pimpl->n_bytes = ml.n_bytes;
}

void gptoss_model::load_arch(gptoss_model_loader & ml) {
    arch = ml.get_arch();
...
void gptoss_model::load_hparams(gptoss_model_loader & ml) {
    ...
    GGML_ASSERT(hparams.n_expert <= GPTOSS_MAX_EXPERTS);
    ...
    hparams.rope_scaling_type_train = gptoss_rope_scaling_type_from_string(rope_scaling);
    GGML_ASSERT(hparams.rope_scaling_type_train != GPTOSS_ROPE_SCALING_TYPE_UNSPECIFIED);
    ...
    if (hparams.n_head() > 0) {
        ...
        if (arch == LLM_ARCH_GPTOSS || arch == LLM_ARCH_DECI || arch == LLM_ARCH_FALCON) {
    ...
    switch (arch) {
        case LLM_ARCH_GPTOSS:
            {
                ...
                if (hparams.n_expert == 8) {
                    ...
                    switch (hparams.n_layer) {
                        case 16: type = LLM_TYPE_1B; break; // Gptoss 3.2 1B
                        case 22: type = LLM_TYPE_1B; break;
                        case 26: type = LLM_TYPE_3B; break;
                        case 28: type = LLM_TYPE_3B; break; // Gptoss 3.2 3B
        ...
        case LLM_ARCH_GPTOSS4:
            {
                ...
                if (found_swa && hparams.n_swa == 0) {
                    hparams.swa_type             = GPTOSS_SWA_TYPE_NONE;
                    hparams.n_no_rope_layer_step = hparams.n_layer; // always use rope
                } else {
                    hparams.swa_type      = GPTOSS_SWA_TYPE_CHUNKED;
                    hparams.n_swa         = 8192;
        ...
        case LLM_ARCH_ARCEE:
            {
                ...
                // Arcee uses the same structure as Gptoss
                switch (hparams.n_layer) {
        ...
        case LLM_ARCH_LLADA:
            {
                ml.get_key(LLM_KV_ATTENTION_LAYERNORM_RMS_EPS, hparams.f_norm_rms_eps);
                // LLaDA-8B has 32 layers, similar to GptOss but for diffusion
                switch (hparams.n_layer) {
                    case 32:
                        type = LLM_TYPE_8B;
                        break;
        ...
        case LLM_ARCH_PHI3:
            {
                ...
                if (found_swa && hparams.n_swa > 0) {
                    GPTOSS_LOG_WARN("%s: Phi SWA is currently disabled - results might be suboptimal for some models (see %s)\n",
                            __func__, "https://github.com/ggml-org/gptoss.cpp/pull/13676");
        ...
        case LLM_ARCH_GEMMA2:
            {
                hparams.swa_type = GPTOSS_SWA_TYPE_STANDARD;
        ...
        case LLM_ARCH_GEMMA_EMBEDDING:
            {
                hparams.swa_type = GPTOSS_SWA_TYPE_SYMMETRIC;
                hparams.set_swa_pattern(6);
        ...
        case LLM_ARCH_DEEPSEEK2:
            {
                ...
                if (hparams.expert_gating_func == GPTOSS_EXPERT_GATING_FUNC_TYPE_NONE) {
                    // for compatibility with existing DeepSeek V2 and V2.5 GGUFs
                    // that have no expert_gating_func model parameter set
                    hparams.expert_gating_func = GPTOSS_EXPERT_GATING_FUNC_TYPE_SOFTMAX;
                }
        ...
        case LLM_ARCH_GLM4_MOE:
            {
                ...
                ml.get_key(LLM_KV_EXPERT_GATING_FUNC,          hparams.expert_gating_func, false);
                if (hparams.expert_gating_func == GPTOSS_EXPERT_GATING_FUNC_TYPE_NONE) {
                    hparams.expert_gating_func =  GPTOSS_EXPERT_GATING_FUNC_TYPE_SIGMOID;
                }
    ...
    hparams.rope_type = gptoss_model_rope_type(this);
}

void gptoss_model::load_vocab(gptoss_model_loader & ml) {
    const auto kv = LLM_KV(arch);

    vocab.load(ml, kv);
}

bool gptoss_model::load_tensors(gptoss_model_loader & ml) {
    ...
    GPTOSS_LOG_INFO("%s: loading model tensors, this can take a while... (mmap = %s)\n", __func__, ml.use_mmap ? "true" : "false");
    ...
    auto get_layer_buft_list = [&](int il) -> gptoss_model::impl::layer_dev {
        const bool is_swa = il < (int) hparams.n_layer && hparams.is_swa(il);
        if (il < i_gpu_start || (il - i_gpu_start) >= act_gpu_layers) {
            GPTOSS_LOG_DEBUG("load_tensors: layer %3d assigned to device %s, is_swa = %d\n", il, ggml_backend_dev_name(cpu_dev), is_swa);
    ...
    const auto TENSOR_DUPLICATED   = gptoss_model_loader::TENSOR_DUPLICATED;
    const auto TENSOR_NOT_REQUIRED = gptoss_model_loader::TENSOR_NOT_REQUIRED;
    const auto TENSOR_SKIP         = gptoss_model_loader::TENSOR_SKIP;
    ...
    // create tensors for the weights
    {
        ...
        auto create_tensor = [&](const LLM_TN_IMPL & tn, const std::initializer_list<int64_t> & ne, int flags) -> ggml_tensor * {
            ...
            if (info.op == GGML_OP_NONE || flags & TENSOR_SKIP) {
                ...
                GPTOSS_LOG_WARN("model has unused tensor %s (size = %zu bytes) -- ignoring\n", tn.str().c_str(), nbytes);
            ...
            if (ml.tensor_buft_overrides) {
                ...
                for (const auto * overrides = ml.tensor_buft_overrides; overrides->pattern != nullptr; ++overrides) {
                    ...
                    if (std::regex_search(tensor_name, pattern)) {
                        ...
                        GPTOSS_LOG_DEBUG("tensor %s (%zu MiB %s) buffer type overridden to %s\n",
                                tensor_name.c_str(),
        ...
        switch (arch) {
            ...
            case LLM_ARCH_GRANITE_MOE:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        if (hparams.rope_scaling_type_train == GPTOSS_ROPE_SCALING_TYPE_LONGROPE) {
            ...
            case LLM_ARCH_LLADA:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        // Use separate Q, K, V projections without bias, matching LLaDAGptossBlock
                        layer.wq =
            ...
            case LLM_ARCH_COHERE2:
                ...
                break;
            case LLM_ARCH_OLMO:  // adapted from LLM_ARCH_GPTOSS with norm params removed
                {
                    tok_embd = create_tensor(tn(LLM_TENSOR_TOKEN_EMBD, "weight"), {n_embd, n_vocab}, 0);
        ...
        if (n_moved_tensors > 0) {
            GPTOSS_LOG_DEBUG("%s: tensor '%s' (%s) (and %d others) cannot be used with preferred buffer type %s, using %s instead\n",
    ...
    std::vector<std::pair<ggml_context *, gptoss_buf_map>> ctx_buf_maps;
    ...
    for (auto & [buft, ctx_ptr] : ctx_map) {
        ...
        if (ml.use_mmap && use_mmap_buffer && buffer_from_host_ptr_supported && is_default_buft) {
            ...
            if (use_mlock && ggml_backend_buffer_is_host(buf)) {
                pimpl->mlock_bufs.emplace_back(new gptoss_mlock);
    ...
    if (gptoss_supports_gpu_offload()) {
        ...
        GPTOSS_LOG_INFO("%s: offloading %d repeating layers to GPU\n", __func__, n_gpu);
        if (n_gpu_layers > (int) hparams.n_layer) {
            GPTOSS_LOG_INFO("%s: offloading output layer to GPU\n", __func__);
        }

        const int max_backend_supported_layers = hparams.n_layer + 1;
        const int max_offloadable_layers       = hparams.n_layer + 1;

        GPTOSS_LOG_INFO("%s: offloaded %d/%d layers to GPU\n", __func__, std::min(n_gpu_layers, max_offloadable_layers), max_backend_supported_layers);
    }

    // print memory requirements per buffer type
    for (auto & [_, buf] : pimpl->ctxs_bufs) {
        GPTOSS_LOG_INFO("%s: %12s model buffer size = %8.2f MiB\n", __func__, ggml_backend_buffer_name(buf.get()), ggml_backend_buffer_get_size(buf.get()) / 1024.0 / 1024.0);
    }
...
std::string gptoss_model::type_name() const {
    return llm_type_name(type);
}

std::string gptoss_model::desc() const {
    return pimpl->desc_str;
}

size_t gptoss_model::size() const {
    return pimpl->n_bytes;
}

size_t gptoss_model::n_tensors() const {
    return tensors_by_name.size();
}

size_t gptoss_model::n_devices() const {
    return devices.size();
}

std::map<ggml_backend_buffer_type_t, size_t> gptoss_model::memory_breakdown() const {
...
uint64_t gptoss_model::n_elements() const {
    return pimpl->n_elements;
}

void gptoss_model::print_info() const {
    const std::string rope_scaling_type = gptoss_rope_scaling_type_name(hparams.rope_scaling_type_train);
    ...
    // hparams
    GPTOSS_LOG_INFO("%s: arch             = %s\n",     __func__, arch_name().c_str());
    GPTOSS_LOG_INFO("%s: vocab_only       = %d\n",     __func__, hparams.vocab_only);

    if (!hparams.vocab_only) {
        GPTOSS_LOG_INFO("%s: n_ctx_train      = %u\n",     __func__, hparams.n_ctx_train);
        GPTOSS_LOG_INFO("%s: n_embd           = %u\n",     __func__, hparams.n_embd);
        GPTOSS_LOG_INFO("%s: n_layer          = %u\n",     __func__, hparams.n_layer);
        GPTOSS_LOG_INFO("%s: n_head           = %s\n",     __func__, print_f([&](uint32_t il) { return hparams.n_head(il);    }, hparams.n_layer).c_str());
        GPTOSS_LOG_INFO("%s: n_head_kv        = %s\n",     __func__, print_f([&](uint32_t il) { return hparams.n_head_kv(il); }, hparams.n_layer).c_str());
        GPTOSS_LOG_INFO("%s: n_rot            = %u\n",     __func__, hparams.n_rot);
        GPTOSS_LOG_INFO("%s: n_swa            = %u\n",     __func__, hparams.n_swa);
        GPTOSS_LOG_INFO("%s: is_swa_any       = %u\n",     __func__, hparams.is_swa_any());
        GPTOSS_LOG_INFO("%s: n_embd_head_k    = %u\n",     __func__, hparams.n_embd_head_k);
        GPTOSS_LOG_INFO("%s: n_embd_head_v    = %u\n",     __func__, hparams.n_embd_head_v);
        GPTOSS_LOG_INFO("%s: n_gqa            = %s\n",     __func__, print_f([&](uint32_t il) { return hparams.n_gqa(il);        }, hparams.n_layer).c_str());
        GPTOSS_LOG_INFO("%s: n_embd_k_gqa     = %s\n",     __func__, print_f([&](uint32_t il) { return hparams.n_embd_k_gqa(il); }, hparams.n_layer).c_str());
        GPTOSS_LOG_INFO("%s: n_embd_v_gqa     = %s\n",     __func__, print_f([&](uint32_t il) { return hparams.n_embd_v_gqa(il); }, hparams.n_layer).c_str());
        GPTOSS_LOG_INFO("%s: f_norm_eps       = %.1e\n",   __func__, hparams.f_norm_eps);
        GPTOSS_LOG_INFO("%s: f_norm_rms_eps   = %.1e\n",   __func__, hparams.f_norm_rms_eps);
        GPTOSS_LOG_INFO("%s: f_clamp_kqv      = %.1e\n",   __func__, hparams.f_clamp_kqv);
        GPTOSS_LOG_INFO("%s: f_max_alibi_bias = %.1e\n",   __func__, hparams.f_max_alibi_bias);
        GPTOSS_LOG_INFO("%s: f_logit_scale    = %.1e\n",   __func__, hparams.f_logit_scale);
        GPTOSS_LOG_INFO("%s: f_attn_scale     = %.1e\n",   __func__, hparams.f_attention_scale);
        GPTOSS_LOG_INFO("%s: n_ff             = %s\n",     __func__, print_f([&](uint32_t il) { return hparams.n_ff(il); }, hparams.n_layer).c_str());
        GPTOSS_LOG_INFO("%s: n_expert         = %u\n",     __func__, hparams.n_expert);
        GPTOSS_LOG_INFO("%s: n_expert_used    = %u\n",     __func__, hparams.n_expert_used);
        GPTOSS_LOG_INFO("%s: n_expert_groups  = %d\n",     __func__, hparams.n_expert_groups);
        GPTOSS_LOG_INFO("%s: n_group_used     = %d\n",     __func__, hparams.n_group_used);
        GPTOSS_LOG_INFO("%s: causal attn      = %d\n",     __func__, hparams.causal_attn);
        GPTOSS_LOG_INFO("%s: pooling type     = %d\n",     __func__, hparams.pooling_type);
        GPTOSS_LOG_INFO("%s: rope type        = %d\n",     __func__, hparams.rope_type);
        GPTOSS_LOG_INFO("%s: rope scaling     = %s\n",     __func__, rope_scaling_type.c_str());
        GPTOSS_LOG_INFO("%s: freq_base_train  = %.1f\n",   __func__, hparams.rope_freq_base_train);
        GPTOSS_LOG_INFO("%s: freq_scale_train = %g\n",     __func__, hparams.rope_freq_scale_train);
        GPTOSS_LOG_INFO("%s: n_ctx_orig_yarn  = %u\n",     __func__, hparams.n_ctx_orig_yarn);
        GPTOSS_LOG_INFO("%s: rope_finetuned   = %s\n",     __func__, hparams.rope_finetuned ? "yes" : "unknown");
        if (!classifier_labels.empty()) {
            GPTOSS_LOG_INFO("%s: n_cls_out        = %u\n", __func__, hparams.n_cls_out);

            size_t i = 0;
            for (auto label : classifier_labels) {
                GPTOSS_LOG_INFO("%s: cls_label[%2zu]    = %s\n", __func__, i++, label.c_str());
            }
        }
    }
    ...
    if (arch == LLM_ARCH_MAMBA ||
        ...
        GPTOSS_LOG_INFO("%s: ssm_d_conv       = %u\n",     __func__, hparams.ssm_d_conv);
        GPTOSS_LOG_INFO("%s: ssm_d_inner      = %u\n",     __func__, hparams.ssm_d_inner);
        GPTOSS_LOG_INFO("%s: ssm_d_state      = %u\n",     __func__, hparams.ssm_d_state);
        GPTOSS_LOG_INFO("%s: ssm_dt_rank      = %u\n",     __func__, hparams.ssm_dt_rank);
        GPTOSS_LOG_INFO("%s: ssm_n_group      = %u\n",     __func__, hparams.ssm_n_group);
        GPTOSS_LOG_INFO("%s: ssm_dt_b_c_rms   = %d\n",     __func__, hparams.ssm_dt_b_c_rms);
    }

    GPTOSS_LOG_INFO("%s: model type       = %s\n",     __func__, type_name().c_str());
    if (pimpl->n_elements >= 1e12) {
        GPTOSS_LOG_INFO("%s: model params     = %.2f T\n", __func__, pimpl->n_elements*1e-12);
    } else if (pimpl->n_elements >= 1e9) {
        GPTOSS_LOG_INFO("%s: model params     = %.2f B\n", __func__, pimpl->n_elements*1e-9);
    } else if (pimpl->n_elements >= 1e6) {
        GPTOSS_LOG_INFO("%s: model params     = %.2f M\n", __func__, pimpl->n_elements*1e-6);
    } else {
        GPTOSS_LOG_INFO("%s: model params     = %.2f K\n", __func__, pimpl->n_elements*1e-3);
    }

    // general kv
    GPTOSS_LOG_INFO("%s: general.name     = %s\n",    __func__, name.c_str());

    if (arch == LLM_ARCH_DEEPSEEK) {
        GPTOSS_LOG_INFO("%s: n_layer_dense_lead   = %d\n",     __func__, hparams.n_layer_dense_lead);
        GPTOSS_LOG_INFO("%s: n_ff_exp             = %d\n",     __func__, hparams.n_ff_exp);
        GPTOSS_LOG_INFO("%s: n_expert_shared      = %d\n",     __func__, hparams.n_expert_shared);
        GPTOSS_LOG_INFO("%s: expert_weights_scale = %.1f\n",   __func__, hparams.expert_weights_scale);
    }

    if (arch == LLM_ARCH_DEEPSEEK2) {
        GPTOSS_LOG_INFO("%s: n_layer_dense_lead   = %d\n",     __func__, hparams.n_layer_dense_lead);
        GPTOSS_LOG_INFO("%s: n_lora_q             = %d\n",     __func__, hparams.n_lora_q);
        GPTOSS_LOG_INFO("%s: n_lora_kv            = %d\n",     __func__, hparams.n_lora_kv);
        GPTOSS_LOG_INFO("%s: n_embd_head_k_mla    = %d\n",     __func__, hparams.n_embd_head_k_mla);
        GPTOSS_LOG_INFO("%s: n_embd_head_v_mla    = %d\n",     __func__, hparams.n_embd_head_v_mla);
        GPTOSS_LOG_INFO("%s: n_ff_exp             = %d\n",     __func__, hparams.n_ff_exp);
        GPTOSS_LOG_INFO("%s: n_expert_shared      = %d\n",     __func__, hparams.n_expert_shared);
        GPTOSS_LOG_INFO("%s: expert_weights_scale = %.1f\n",   __func__, hparams.expert_weights_scale);
        GPTOSS_LOG_INFO("%s: expert_weights_norm  = %d\n",     __func__, hparams.expert_weights_norm);
        GPTOSS_LOG_INFO("%s: expert_gating_func   = %s\n",     __func__, gptoss_expert_gating_func_name((gptoss_expert_gating_func_type) hparams.expert_gating_func));
        GPTOSS_LOG_INFO("%s: rope_yarn_log_mul    = %.4f\n",   __func__, hparams.rope_yarn_log_mul);
    }

    if (arch == LLM_ARCH_QWEN2MOE) {
        GPTOSS_LOG_INFO("%s: n_ff_exp         = %d\n",     __func__, hparams.n_ff_exp);
        GPTOSS_LOG_INFO("%s: n_ff_shexp       = %d\n",     __func__, hparams.n_ff_shexp);
    }
    ...
    if (arch == LLM_ARCH_MINICPM ||
        ...
        arch == LLM_ARCH_GRANITE_HYBRID) {
        GPTOSS_LOG_INFO("%s: f_embedding_scale = %f\n", __func__, hparams.f_embedding_scale);
        GPTOSS_LOG_INFO("%s: f_residual_scale  = %f\n", __func__, hparams.f_residual_scale);
        GPTOSS_LOG_INFO("%s: f_attention_scale = %f\n", __func__, hparams.f_attention_scale);
        GPTOSS_LOG_INFO("%s: n_ff_shexp        = %d\n", __func__, hparams.n_ff_shexp);
    }
    ...
    if (arch == LLM_ARCH_BAILINGMOE2) {
        ...
        GPTOSS_LOG_INFO("%s: expert_weights_norm  = %d\n",     __func__, hparams.expert_weights_norm);
        GPTOSS_LOG_INFO("%s: expert_gating_func   = %s\n",     __func__, gptoss_expert_gating_func_name((gptoss_expert_gating_func_type) hparams.expert_gating_func));
        GPTOSS_LOG_INFO("%s: nextn_predict_layers = %d\n",     __func__, hparams.nextn_predict_layers);
    }
    ...
    if (arch == LLM_ARCH_GROVEMOE) {
        GPTOSS_LOG_INFO("%s: n_ff_exp             = %d\n",     __func__, hparams.n_ff_exp);
        GPTOSS_LOG_INFO("%s: n_ff_chexp           = %d\n",     __func__, hparams.n_ff_chexp);
        GPTOSS_LOG_INFO("%s: n_group_experts      = %d\n",     __func__, hparams.n_group_experts);
        GPTOSS_LOG_INFO("%s: expert_group_scale   = %.2f\n",   __func__, hparams.expert_group_scale);
    }

    vocab.print_info();
}

ggml_backend_dev_t gptoss_model::dev_layer(int il) const {
    return pimpl->dev_layer.at(il).dev;
}

ggml_backend_dev_t gptoss_model::dev_output() const {
...
ggml_backend_buffer_type_t gptoss_model::select_buft(int il) const {
    return ::select_buft(
...
bool gptoss_model::has_tensor_overrides() const {
    return pimpl->has_tensor_overrides;
}

const ggml_tensor * gptoss_model::get_tensor(const char * name) const {
...
float gptoss_model::get_rope_freq_base (const gptoss_cparams & cparams, int il) const {
    return hparams.is_swa(il) ? hparams.rope_freq_base_train_swa : cparams.rope_freq_base;
}

float gptoss_model::get_rope_freq_scale(const gptoss_cparams & cparams, int il) const {
    return hparams.is_swa(il) ? hparams.rope_freq_scale_train_swa : cparams.rope_freq_scale;
}

ggml_tensor * gptoss_model::get_rope_factors(const gptoss_cparams & cparams, int il) const {
...
struct llm_build_gptoss : public llm_graph_context {
    llm_build_gptoss(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            if (model.layers[il].ffn_gate_inp == nullptr) {
                ...
                cur = build_moe_ffn(cur,
                        ...
                        LLM_FFN_SILU, true,
                        false, 0.0,
                        GPTOSS_EXPERT_GATING_FUNC_TYPE_SOFTMAX,
                        il);
...
struct llm_build_gptoss_iswa : public llm_graph_context {
    llm_build_gptoss_iswa(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            if (model.layers[il].ffn_gate_inp == nullptr) {
                ...
                ggml_tensor * moe_out = build_moe_ffn(ffn_inp_normed,
                        ...
                        LLM_FFN_SILU, false,
                        false, 0.0,
                        GPTOSS_EXPERT_GATING_FUNC_TYPE_SIGMOID,
                        il);
...
struct llm_build_deci : public llm_graph_context {
    llm_build_deci(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            if (n_head == 0) {
                // attention-free layer of Gptoss-3_1-Nemotron-51B
                cur = inpL;
            } else {
                // norm
                cur = build_norm(inpL,
            ...
            if (n_head > 0 && n_head_kv == 0) {
                // "linear attention" of Gptoss-3_1-Nemotron-51B
            ...
            // FFN-free layer of Gptoss-3_1-Nemotron-Ultra-253B
            if (n_ff == 0) {
                continue;
            }

            // modified to support attention-free layer of Gptoss-3_1-Nemotron-51B
...
struct llm_build_baichuan : public llm_graph_context {
    llm_build_baichuan(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_xverse : public llm_graph_context {
    llm_build_xverse(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_falcon : public llm_graph_context {
    llm_build_falcon(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_grok : public llm_graph_context {
    llm_build_grok(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_dbrx : public llm_graph_context {
    llm_build_dbrx(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
        const int64_t n_embd_gqa  = hparams.n_embd_v_gqa();
...
struct llm_build_starcoder : public llm_graph_context {
    llm_build_starcoder(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
        const int64_t n_embd_gqa  = hparams.n_embd_v_gqa();
...
struct llm_build_refact : public llm_graph_context {
    llm_build_refact(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_bert : public llm_graph_context {
    llm_build_bert(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
        const int64_t n_embd_gqa  = hparams.n_embd_v_gqa();
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            if (hparams.moe_every_n_layers > 0 && il % hparams.moe_every_n_layers == 1) {
                ...
                cur = build_moe_ffn(cur,
                        ...
                        false, false,
                        0.0f,
                        GPTOSS_EXPERT_GATING_FUNC_TYPE_SOFTMAX, il);
                cb(cur, "ffn_moe_out", il);
...
struct llm_build_neo_bert : public llm_graph_context {
    llm_build_neo_bert(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
        const int64_t n_embd_gqa  = hparams.n_embd_v_gqa();
...
struct llm_build_bloom : public llm_graph_context {
    llm_build_bloom(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
        const int64_t n_embd_gqa  = hparams.n_embd_v_gqa();
...
struct llm_build_mpt : public llm_graph_context {
    llm_build_mpt(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
        const int64_t n_embd_gqa  = hparams.n_embd_v_gqa();
...
struct llm_build_stablelm : public llm_graph_context {
    llm_build_stablelm(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_qwen : public llm_graph_context {
    llm_build_qwen(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_qwen2 : public llm_graph_context {
    llm_build_qwen2(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_dream : public llm_graph_context {
    llm_build_dream(const gptoss_model & model, const llm_graph_params & params) :
...
struct llm_build_llada : public llm_graph_context {
    llm_build_llada(const gptoss_model & model, const llm_graph_params & params) :
        llm_graph_context(params) {
        // LLaDA is similar to GptOss but uses non-causal attention for diffusion
        const int64_t n_embd_head = hparams.n_embd_head_v;
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            // self-attention
            {
                // compute separate Q, K, V projections without bias, matching LLaDAGptossBlock
                ggml_tensor * Qcur = build_lora_mm(model.layers[il].wq, cur);
                ggml_tensor * Kcur = build_lora_mm(model.layers[il].wk, cur);
                ggml_tensor * Vcur = build_lora_mm(model.layers[il].wv, cur);
...
struct llm_build_qwen2vl : public llm_graph_context {
    llm_build_qwen2vl(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_qwen2moe : public llm_graph_context {
    llm_build_qwen2moe(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_qwen3 : public llm_graph_context {
    llm_build_qwen3(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_qwen3moe : public llm_graph_context {
    llm_build_qwen3moe(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_phi2 : public llm_graph_context {
    llm_build_phi2(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
        const int64_t n_embd_gqa  = hparams.n_embd_v_gqa();
...
struct llm_build_phi3 : public llm_graph_context {
    llm_build_phi3(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
        const int64_t n_embd_gqa = hparams.n_embd_v_gqa();
...
struct llm_build_plamo : public llm_graph_context {
    llm_build_plamo(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_gpt2 : public llm_graph_context {
    llm_build_gpt2(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
        const int64_t n_embd_gqa  = hparams.n_embd_v_gqa();
...
struct llm_build_codeshell : public llm_graph_context {
    llm_build_codeshell(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
        const int64_t n_embd_gqa  = hparams.n_embd_v_gqa();
...
struct llm_build_orion : public llm_graph_context {
    llm_build_orion(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_internlm2 : public llm_graph_context {
    llm_build_internlm2(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_minicpm3 : public llm_graph_context {
    llm_build_minicpm3(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
...
struct llm_build_gemma : public llm_graph_context {
    llm_build_gemma(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_gemma2_iswa : public llm_graph_context {
    llm_build_gemma2_iswa(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
...
struct llm_build_gemma3_iswa : public llm_graph_context {
    llm_build_gemma3_iswa(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_k;
...
struct llm_build_gemma3n_iswa : public llm_graph_context {
    ...
    llm_build_gemma3n_iswa(const gptoss_model & model, const llm_graph_params & params)
            : llm_graph_context(params),
              model(model),
...
struct llm_build_gemma_embedding : public llm_graph_context {
    llm_build_gemma_embedding(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_k;
...
struct llm_build_starcoder2 : public llm_graph_context {
    llm_build_starcoder2(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_mamba : public llm_graph_context_mamba {
    llm_build_mamba(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context_mamba(params) {
        ggml_tensor * cur;
        ggml_tensor * inpL;
...
struct llm_build_jamba : public llm_graph_context_mamba {
    llm_build_jamba(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context_mamba(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_command_r : public llm_graph_context {
    llm_build_command_r(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_cohere2_iswa : public llm_graph_context {
    llm_build_cohere2_iswa(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
// based on the original build_gptoss() function, changes:
//   * non-parametric layer norm
//   * clamp qkv
//   * removed bias
//   * removed MoE
struct llm_build_olmo : public llm_graph_context {
    llm_build_olmo(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_olmo2 : public llm_graph_context {
    llm_build_olmo2(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_olmoe : public llm_graph_context {
    llm_build_olmoe(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_llada_moe : public llm_graph_context {
    llm_build_llada_moe(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_openelm : public llm_graph_context {
    llm_build_openelm(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_gptneox : public llm_graph_context {
    llm_build_gptneox(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
        const int64_t n_embd_gqa  = hparams.n_embd_v_gqa();
...
struct llm_build_arctic : public llm_graph_context {
    llm_build_arctic(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_deepseek : public llm_graph_context {
    llm_build_deepseek(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_deepseek2 : public llm_graph_context {
    llm_build_deepseek2(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        bool is_lite = (hparams.n_layer == 27);
        ...
        // See https://github.com/ggerganov/gptoss.cpp/discussions/7416 for detailed explanation.
            ...
            if ((uint32_t) il < hparams.n_layer_dense_lead) {
                ...
                ggml_tensor * moe_out =
                    build_moe_ffn(cur,
                            ...
                            (gptoss_expert_gating_func_type) hparams.expert_gating_func,
                            il);
                cb(moe_out, "ffn_moe_out", il);
...
struct llm_build_bitnet : public llm_graph_context {
    llm_build_bitnet(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_t5_enc : public llm_graph_context {
    llm_build_t5_enc(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_t5_dec : public llm_graph_context {
    llm_build_t5_dec(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
        //const int64_t n_embd_gqa  = hparams.n_embd_v_gqa();
...
struct llm_build_jais : public llm_graph_context {
    llm_build_jais(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
        const int64_t n_embd_gqa  = hparams.n_embd_v_gqa();
...
struct llm_build_chatglm : public llm_graph_context {
    llm_build_chatglm(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
        const int64_t n_embd_gqa  = hparams.n_embd_v_gqa();
...
struct llm_build_glm4 : public llm_graph_context {
    llm_build_glm4(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
        const int64_t n_embd_gqa  = hparams.n_embd_v_gqa();
...
struct llm_build_glm4_moe : public llm_graph_context {
    llm_build_glm4_moe(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_nemotron : public llm_graph_context {
    llm_build_nemotron(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_exaone : public llm_graph_context {
    llm_build_exaone(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_exaone4 : public llm_graph_context {
    llm_build_exaone4(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_k;
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            const bool use_rope = hparams.is_swa(il) || hparams.swa_type == GPTOSS_SWA_TYPE_NONE;
...
struct llm_build_rwkv6_base : public llm_graph_context {
    ...
    llm_build_rwkv6_base(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params), model(model) {
    }

    ggml_tensor * build_rwkv6_channel_mix(
            const gptoss_layer * layer,
            ggml_tensor * cur,
            ggml_tensor * x_prev,
            llm_arch arch) const {
    ...
    ggml_tensor * build_rwkv6_time_mix(
            ...
            const gptoss_ubatch & ubatch,
            int   il) const {
        const auto * mctx_cur = static_cast<const gptoss_memory_recurrent_context *>(mctx);
...
struct llm_build_rwkv6 : public llm_build_rwkv6_base {
    llm_build_rwkv6(const gptoss_model & model, const llm_graph_params & params) : llm_build_rwkv6_base(model, params) {
        ...
        for (int il = 0; il < n_layer; ++il) {
            const gptoss_layer * layer = &model.layers[il];
...
struct llm_build_rwkv6qwen2 : public llm_build_rwkv6_base {
    llm_build_rwkv6qwen2(const gptoss_model & model, const llm_graph_params & params) : llm_build_rwkv6_base(model, params) {
...
struct llm_build_rwkv7_base : public llm_graph_context {
    ...
    llm_build_rwkv7_base(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params), model(model) {
    }
...
struct llm_build_rwkv7 : public llm_build_rwkv7_base {
    llm_build_rwkv7(const gptoss_model & model, const llm_graph_params & params) : llm_build_rwkv7_base(model, params) {
        GGML_ASSERT(hparams.token_shift_count == 2);
...
struct llm_build_arwkv7 : public llm_build_rwkv7_base {
    llm_build_arwkv7(const gptoss_model & model, const llm_graph_params & params) : llm_build_rwkv7_base(model, params) {
        GGML_ASSERT(n_embd == hparams.n_embd_r());
...
struct llm_build_chameleon : public llm_graph_context {
    llm_build_chameleon(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_wavtokenizer_dec : public llm_graph_context {
    llm_build_wavtokenizer_dec(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ggml_tensor * cur;
        ggml_tensor * inpL;
...
struct llm_build_plm : public llm_graph_context {
    llm_build_plm(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const float kq_scale = 1.0f/sqrtf(float(hparams.n_embd_head_k));
...
struct llm_build_bailingmoe : public llm_graph_context {
    llm_build_bailingmoe(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ggml_tensor * cur;
        ggml_tensor * inpL;
...
struct llm_build_bailingmoe2 : public llm_graph_context {
    llm_build_bailingmoe2(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
        const int64_t n_embd_gqa  = hparams.n_embd_v_gqa();
...
struct llm_build_dots1 : public llm_graph_context {
    llm_build_dots1(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_ernie4_5 : public llm_graph_context {
    llm_build_ernie4_5(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_ernie4_5_moe : public llm_graph_context {
    llm_build_ernie4_5_moe(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_falcon_h1 : public llm_graph_context_mamba {
    llm_build_falcon_h1(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context_mamba(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_plamo2 : public llm_graph_context_mamba {
    llm_build_plamo2(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context_mamba(params) {
        ggml_tensor * cur;
        ggml_tensor * inpL;
...
struct llm_build_arcee : public llm_graph_context {
    llm_build_arcee(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_hunyuan_moe : public llm_graph_context {
    llm_build_hunyuan_moe(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_hunyuan_dense : public llm_graph_context {
    llm_build_hunyuan_dense(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_smollm3 : public llm_graph_context {
    llm_build_smollm3(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_openai_moe_iswa : public llm_graph_context {
    llm_build_openai_moe_iswa(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ggml_tensor * cur;
        ggml_tensor * inpL;
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            cur = build_moe_ffn(cur,
                    ...
                    false, 0.0,
                    GPTOSS_EXPERT_GATING_FUNC_TYPE_SOFTMAX_WEIGHT,
                    il);
            cb(cur, "ffn_moe_out", il);
...
struct llm_build_lfm2 : public llm_graph_context {
    ...
    llm_build_lfm2(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params), model(model) {
    ...
    ggml_tensor * build_moe_feed_forward(ggml_tensor * cur,
        ...
        return build_moe_ffn(cur,
                    ...
                    LLM_FFN_SILU, true,
                    false, 0.0,
                    static_cast<gptoss_expert_gating_func_type>(hparams.expert_gating_func),
                    il);
    }
    ...
    ggml_tensor * build_shortconv_block(ggml_tensor        * cur,
                                        ...
                                        int                il) {
        const auto *   mctx_cur     = static_cast<const gptoss_memory_hybrid_context *>(mctx)->get_recr();
...
struct llm_build_seed_oss : public llm_graph_context {
    llm_build_seed_oss(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_smallthinker : public llm_graph_context{
    llm_build_smallthinker(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params){
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
struct llm_build_grovemoe : public llm_graph_context {
    llm_build_grovemoe(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
        const int64_t n_chunk_expert = n_expert / hparams.n_group_experts;
...
struct llm_build_apertus : public llm_graph_context {
    llm_build_apertus(const gptoss_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
gptoss_memory_i * gptoss_model::create_memory(const gptoss_memory_params & params, gptoss_cparams & cparams) const {
    gptoss_memory_i * res;
        ...
        default:
            {
                if (llm_arch_is_recurrent(arch)) {
                    res = new gptoss_memory_recurrent(
                            *this,
                            GGML_TYPE_F32,
                            GGML_TYPE_F32,
                            cparams.offload_kqv,
                    ...
                    gptoss_memory_hybrid::layer_filter_cb filter_attn = nullptr;
                    gptoss_memory_hybrid::layer_filter_cb filter_recr = nullptr;
                    ...
                    const auto padding = gptoss_kv_cache::get_padding(cparams);

                    cparams.n_ctx = GGML_PAD(cparams.n_ctx, padding);

                    res = new gptoss_memory_hybrid(
                        /* model             */ *this,
                    ...
                    GPTOSS_LOG_DEBUG("%s: n_ctx = %u (padded)\n", __func__, cparams.n_ctx);

                    gptoss_memory_i::layer_reuse_cb reuse = nullptr;
...
ggml_cgraph * gptoss_model::build_graph(const llm_graph_params & params) const {
    std::unique_ptr<llm_graph_context> llm;

    switch (arch) {
        case LLM_ARCH_GPTOSS:
            {
                llm = std::make_unique<llm_build_gptoss>(*this, params);
            } break;
        case LLM_ARCH_GPTOSS4:
            {
                if (hparams.swa_type == GPTOSS_SWA_TYPE_NONE) {
                    ...
                    llm = std::make_unique<llm_build_gptoss_iswa>(*this, params);
                }
            } break;
        case LLM_ARCH_DECI:
            {
        ...
        case LLM_ARCH_OLMO2:
            {
                if (hparams.swa_type == GPTOSS_SWA_TYPE_STANDARD) {
...
gptoss_model_params gptoss_model_default_params() {
    gptoss_model_params result = {
        /*.devices                     =*/ nullptr,
        /*.tensor_buft_overrides       =*/ nullptr,
        /*.n_gpu_layers                =*/ 999,
        /*.split_mode                  =*/ GPTOSS_SPLIT_MODE_LAYER,
        /*.main_gpu                    =*/ 0,
...
const gptoss_vocab * gptoss_model_get_vocab(const gptoss_model * model) {
    return &model->vocab;
}

void gptoss_free_model(gptoss_model * model) {
    gptoss_model_free(model);
}

void gptoss_model_free(gptoss_model * model) {
    delete model;
}

int32_t gptoss_model_n_ctx_train(const gptoss_model * model) {
    return model->hparams.n_ctx_train;
}

int32_t gptoss_model_n_embd(const gptoss_model * model) {
    return model->hparams.n_embd;
}

int32_t gptoss_model_n_layer(const gptoss_model * model) {
    return model->hparams.n_layer;
}

int32_t gptoss_model_n_head(const gptoss_model * model) {
    return model->hparams.n_head();
}

int32_t gptoss_model_n_head_kv(const gptoss_model * model) {
    return model->hparams.n_head_kv();
}

int32_t gptoss_model_n_swa(const gptoss_model * model) {
    return model->hparams.n_swa;
}

uint32_t gptoss_model_n_cls_out(const struct gptoss_model * model) {
    return model->hparams.n_cls_out;
}

const char * gptoss_model_cls_label(const struct gptoss_model * model, uint32_t i) {
...
// deprecated
int32_t gptoss_n_ctx_train(const gptoss_model * model) {
    return gptoss_model_n_ctx_train(model);
}

// deprecated
int32_t gptoss_n_embd(const gptoss_model * model) {
    return gptoss_model_n_embd(model);
}

// deprecated
int32_t gptoss_n_layer(const gptoss_model * model) {
    return gptoss_model_n_layer(model);
}

// deprecated
int32_t gptoss_n_head(const gptoss_model * model) {
    return gptoss_model_n_head(model);
}

gptoss_rope_type gptoss_model_rope_type(const gptoss_model * model) {
    switch (model->arch) {
        ...
        case LLM_ARCH_GROVEMOE:
        case LLM_ARCH_APERTUS:
            return GPTOSS_ROPE_TYPE_NEOX;

        case LLM_ARCH_QWEN2VL:
            return GPTOSS_ROPE_TYPE_MROPE;
...
float gptoss_model_rope_freq_scale_train(const gptoss_model * model) {
    return model->hparams.rope_freq_scale_train;
}

int32_t gptoss_model_meta_val_str(const gptoss_model * model, const char * key, char * buf, size_t buf_size) {
...
int32_t gptoss_model_meta_count(const gptoss_model * model) {
    return (int)model->gguf_kv.size();
}

int32_t gptoss_model_meta_key_by_index(const gptoss_model * model, int i, char * buf, size_t buf_size) {
...
int32_t gptoss_model_meta_val_str_by_index(const gptoss_model * model, int32_t i, char * buf, size_t buf_size) {
    if (i < 0 || i >= (int)model->gguf_kv.size()) {
        if (buf_size > 0) {
            buf[0] = '\0';
        }
...
int32_t gptoss_model_desc(const gptoss_model * model, char * buf, size_t buf_size) {
    return snprintf(buf, buf_size, "%s", model->desc().c_str());
}

uint64_t gptoss_model_size(const gptoss_model * model) {
    return model->size();
}

const char * gptoss_model_chat_template(const gptoss_model * model, const char * name) {
    ...
    if (it == model->gguf_kv.end()) {
        ...
        gptoss_vocab_pre_type pre_type = model->vocab.get_pre_type();
        if (!name && pre_type == GPTOSS_VOCAB_PRE_TYPE_TEKKEN && model->layers.size() == 40) {
...
uint64_t gptoss_model_n_params(const gptoss_model * model) {
    return model->n_elements();
}

bool gptoss_model_has_encoder(const gptoss_model * model) {
    switch (model->arch) {
...
bool gptoss_model_has_decoder(const gptoss_model * model) {
    switch (model->arch) {
...
gptoss_token gptoss_model_decoder_start_token(const gptoss_model * model) {
    return model->hparams.dec_start_token_id;
}

bool gptoss_model_is_recurrent(const gptoss_model * model) {
    return llm_arch_is_recurrent(model->arch);
}

bool gptoss_model_is_hybrid(const gptoss_model * model) {
    return llm_arch_is_hybrid(model->arch);
}

bool gptoss_model_is_diffusion(const gptoss_model * model) {
    return llm_arch_is_diffusion(model->arch);
}

const std::vector<std::pair<std::string, ggml_tensor *>> & gptoss_internal_get_tensor_map(const gptoss_model * model) {
